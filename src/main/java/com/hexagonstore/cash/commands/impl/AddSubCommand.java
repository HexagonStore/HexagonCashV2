...
public class AddSubCommand implements ISubCommand {
    ...
    public void execute(DataManager dataManager, HexagonCashAPI cashAPI, EC_Config config, CommandSender s, String[] a) {
        if (s.hasPermission(config.getString("cmd permission.admin"))) {
            if (a.length < 3) {
                s.sendMessage(Language.get("no_args.add"));
                return;
            }
            ...
            try {
                ...
                cashAPI.addCash(account, amount);
                s.sendMessage(Language.get("added").replace("{cash}", NumberFormatter.formatNumber(amount)).replace("{player}", nick));
            } catch (Exception e) {
                s.sendMessage(Language.get("invalid_number"));
            }
        } else {
